/**
 * NextRoute Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead nextroute:// protocol (no handler existed)
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. NextRoute.app/Contents/MacOS/NextRoute). Spawning it with
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "nextroute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "nextroute");

  return path.join(require("os").homedir(), ".nextroute");
}
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.nextroute.online https://*.nextroute.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.NEXTROUTE_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by NextRoute bootstrap",
        "",
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      const isHeadless =
        ...
        process.env.NEXTROUTE_HEADLESS === "true";
      if (isHeadless && !global.loggedHeadlessReady) {
        global.loggedHeadlessReady = true;
        console.log("\n\x1b[32m✔ NextRoute Headless CLI Server is ready and listening!\x1b[0m");
  ...
  try {
    ...
    const desktopFileContent =
      [
        ...
        "Type=Application",
        "Name=NextRoute",
        "Comment=NextRoute Desktop Client",
    ...
    fs.writeFileSync(path.join(autostartDir, "nextroute-desktop.desktop"), desktopFileContent, {
      mode: 0o644,
    });
    return true;
  } catch (err) {
...
function isLinuxDesktopAutostartEnabled() {
  try {
    ...
    return fs.existsSync(
      path.join(os.homedir(), ".config", "autostart", "nextroute-desktop.desktop")
    );
  } catch {
    return false;
  }

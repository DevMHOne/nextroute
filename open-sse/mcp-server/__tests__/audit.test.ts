...
describe("MCP audit shutdown", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    globalThis.__nextrouteMcpAuditDb = undefined;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-mcp-audit-"));
  ...
  it("checkpoints and closes the audit database during shutdown", async () => {
    ...
    await audit.logToolCall("nextroute_get_health", { ok: true }, { ok: true }, 12, true);
  ...
  it("still closes the audit database when checkpoint fails", async () => {
    ...
    await audit.logToolCall("nextroute_get_health", {}, {}, 5, true);
    expect(audit.closeAuditDb()).toBe(true);
    expect(mockDb.close).toHaveBeenCalledTimes(1);
  });
});

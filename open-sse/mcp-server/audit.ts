...
declare global {
  var __nextrouteMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__nextrouteMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__nextrouteMcpAuditDb = database;
}
 ...
 * Uses the same SQLite database as the main NextRoute app.
 */
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".nextroute", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.NEXTROUTE_API_KEY_ID || null;

...
// NextRoute model ID → ChatGPT internal slug. NextRoute uses dot-form IDs
 ...
 *   1. NextRoute dot-form id (`gpt-5.4-thinking-mini`) — every thinking
 ...
 * dozen "NextRoute" entries clutter the sidebar and can interact with
 ...
 * often reach NextRoute from a container while the user's browser needs a
...
function derivePublicBaseUrl(
  ...
  const explicitPublicBase = normalizePublicBaseUrl(process.env.NEXTROUTE_PUBLIC_BASE_URL);
  if (explicitPublicBase) {
    log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: using NEXTROUTE_PUBLIC_BASE_URL`);
    return explicitPublicBase;
  }

  const headerBase = deriveHeaderBaseUrl(clientHeaders);
  const configuredBase =
    normalizePublicBaseUrl(process.env.NEXTROUTE_BASE_URL) ||
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from NextRoute's
 ...
 * and return a browser-fetchable NextRoute URL.
 */
...
function configuredAsyncImageTimeoutMs(): number {
  const raw = Number(process.env.NEXTROUTE_CGPT_WEB_IMAGE_TIMEOUT_MS);
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // NextRoute's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an NextRoute

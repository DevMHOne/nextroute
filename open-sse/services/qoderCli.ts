...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.NEXTROUTE_QODER_WORKSPACE || ""
  ).trim();
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an NextRoute OpenAI-compatible request through the Qoder CLI transport.",
...
export function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "nextroute.user@qoder.sh";
  const userInfo = {
    uid: uid,

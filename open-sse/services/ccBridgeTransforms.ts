 ...
 *
 * Goal: ensure the final request body NextRoute sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - nextroute-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "nextroute-daystamp";
  /**
 ...
 * proven against the live NextRoute deployment (call log
...
/**
 * Compute the `cc_version` suffix per the NextRoute native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    options.versionFormat === "nextroute-daystamp"

...
export const SKIP_UNIVERSAL_HANDOFF_FLAG = "_nextrouteSkipUniversalHandoff";
...
async function generateHandoffAsync(options: {
  ...
  const summaryBody = {
    ...
    temperature: 0.1,
    _nextrouteSkipContextRelay: true,
    _nextrouteInternalRequest: "context-handoff",
  };
...
async function generateUniversalHandoffAsync(options: {
  ...
  const summaryBody: Record<string, unknown> = {
    ...
    temperature: 0.1,
    _nextrouteSkipContextRelay: true,
    _nextrouteInternalRequest: "universal-handoff",
  };

...
export const NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME = "nextroute_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to nextroute_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends JsonRecord>(
  ...
  if (!toolNames.has(NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }

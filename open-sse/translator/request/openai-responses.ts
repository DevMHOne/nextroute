...
const RESPONSES_STORE_MARKER = "_nextrouteResponsesStore";
const COPILOT_REASONING_SUMMARY_MARKER = "_nextrouteCopilotReasoningSummary";
...
export function openaiResponsesToOpenAIRequest(
    ...
    for (const toolValue of tools) {
      ...
      if (
        ...
        throw unsupportedFeature(
          `Unsupported Responses API feature: ${toolType} tool type is not supported by nextroute`
        );
      }
    }
  }
  ...
  // NextRoute is a forward proxy that streams responses synchronously —
  ...
  if (
    ...
    if (tcType === "function" && tc.name !== undefined && !tc.function) {
      ...
      throw unsupportedFeature(
        `Unsupported Responses API feature: tool_choice type '${tcType}' is not supported by nextroute`
      );
    }
  }

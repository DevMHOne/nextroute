...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_nextroute for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_nextroute" : "<YOUR_NEXTROUTE_API_KEY>";

    let configContent = `# NextRoute Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
...
# Utilize the built-in OpenAI provider pointed to NextRoute
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!codexStatus.hasNextRoute}
                  loading={restoring}
                >

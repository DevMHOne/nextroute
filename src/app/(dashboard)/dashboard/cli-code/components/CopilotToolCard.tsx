 ...
 *
 * Feature request: https://github.com/diegosouzapw/NextRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("nextroute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("nextroute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "nextroute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "NextRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.nextroute}`,
      models,
    };

    const responsesConfig = {
      name: "NextRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.nextroute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("nextroute-cli-key-copilot", value);
  };

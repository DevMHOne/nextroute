...
/**
 * Check if the config file contains NextRoute settings.
 */
const hasNextRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  return (
    typeof settings.baseUrl === "string" &&
    settings.baseUrl.length > 0 &&
    settings._managedBy === "nextroute"
  );
};
...
// POST — write NextRoute settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge NextRoute settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove NextRoute settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove NextRoute-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt NextRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }

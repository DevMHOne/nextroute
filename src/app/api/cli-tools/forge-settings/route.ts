...
/**
 * Render the NextRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by NextRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-NextRoute marker comment.
 */
const hasNextRouteConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by NextRoute");
};
...
// POST — write NextRoute settings to Forge config.toml
...
// DELETE — remove Forge NextRoute config

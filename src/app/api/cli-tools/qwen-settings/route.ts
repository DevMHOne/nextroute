...
// Check if settings has NextRoute config
const hasNextRouteConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("NextRoute") || p.id === "nextroute") return true;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasNextRoute: hasNextRouteConfig(settings),
...
// POST - Write NextRoute config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedApiKey = apiKey || "sk_nextroute";
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old NextRoute-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via NextRoute
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (NextRoute)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "nextroute")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via NextRoute
    const anthropicEntry = {
      id: "claude-sonnet-4-6",
      name: "Claude Sonnet 4.6 (NextRoute)",
    ...
    // gemini provider — for Gemini models via NextRoute
    const geminiEntry = {
      id: "gemini-3-flash",
      name: "Gemini 3 Flash (NextRoute)",
...
// DELETE - Remove NextRoute config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove NextRoute entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("NextRoute") && p.id !== "nextroute"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "NextRoute settings removed from Qwen Code",
    });
  } catch (error) {

...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.nextroute]
...
// Check if config has NextRoute settings
const hasNextRouteConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "nextroute"') ||
    config.includes("[model_providers.nextroute]")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasNextRoute: hasNextRouteConfig(config),
...
// POST - Update NextRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only NextRoute related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use NEXTROUTE_API_KEY
    parsed._root.model_provider = "nextroute";
    parsed._sections["model_providers.nextroute"] = {
      name: "NextRoute",
...
// DELETE - Remove NextRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove NextRoute related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "nextroute") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove nextroute provider section
    delete parsed._sections["model_providers.nextroute"];
    ...
    return NextResponse.json({
      success: true,
      message: "NextRoute settings removed successfully",
    });
  } catch (error) {

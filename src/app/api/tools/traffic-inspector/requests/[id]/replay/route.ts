 ...
 *
 * Re-issues the captured request through the local NextRoute instance and
...
const NEXTROUTE_BASE = process.env.NEXTROUTE_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${NEXTROUTE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-nextroute-source": "inspector-replay",
  };

...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.NEXTROUTE_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER = "x-nextroute-ws-bridge-secret";
...
// server-ws.mjs (NEXTROUTE_WS_BRIDGE_SECRET). Without this carve-out the MANAGEMENT
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.NEXTROUTE_WS_BRIDGE_SECRET || "";

 ...
 * can be iframed inside the NextRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between NextRoute and the embedded service.
 ...
 * Prevents NextRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   NextRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the NextRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (NextRoute controls framing via its own CSP).

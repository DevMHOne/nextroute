...
function getLHCState() {
  if (!globalThis.__nextrouteLocalHC) {
    globalThis.__nextrouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__nextrouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("NEXTROUTE_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||

...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push('  -H "Authorization: Bearer $NEXTROUTE_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g nextroute   # or: npx nextroute");
  lines.push("nextroute --version");
  lines.push("```");
  lines.push("");

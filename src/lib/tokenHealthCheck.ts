...
import {
  ...
  refreshCopilotToken,
} from "@nextroute/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("NEXTROUTE_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * NEXTROUTE_DISABLE_TOKEN_HEALTHCHECK switch: it lets an operator keep the
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.NEXTROUTE_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("NEXTROUTE_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __nextrouteTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getHCState() {
  if (!globalThis.__nextrouteTokenHC) {
    globalThis.__nextrouteTokenHC = { initialized: false, interval: null };
  }
  return globalThis.__nextrouteTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in NextRoute

 ...
 * Priority: system daemon socket → NextRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to NextRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `nextroute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");

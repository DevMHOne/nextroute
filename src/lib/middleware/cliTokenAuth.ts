...
const HEADER_NAME = "x-nextroute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local nextroute CLI.
 * Only accepted from loopback IPs. Disabled via NEXTROUTE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.NEXTROUTE_DISABLE_CLI_TOKEN === "true") return false;

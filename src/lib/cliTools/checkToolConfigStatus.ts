...
/**
 * Check if a tool has NextRoute configured by reading its config file directly.
  ...
  try {
    ...
    if (toolId === "hermes") {
      ...
      const hasNextRoute =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasNextRoute ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores NextRoute config differently
    switch (toolId) {
      case "claude":
      ...
      case "cline":
      case "kilo": {
        // Generic check: look for NextRoute-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("nextroute") ||
          configStr.includes("sk_nextroute") ||

...
// ─── Provider mapping: LiteLLM provider → NextRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → NextRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (NextRoute format)
 * Filters: only chat/completion modes (skip image/audio/embedding)
 */
export function transformToNextRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToNextRoute(raw);

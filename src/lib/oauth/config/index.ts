 ...
 * with the running NextRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.NEXTROUTE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.NEXTROUTE_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.NEXTROUTE_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}

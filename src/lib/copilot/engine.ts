...
Use \`createCombo\` tool or \`runNextRouteCli\` to create them.`;
  }
...
NextRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the NextRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## NextRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`nextroute health\`
- "CLI list-combos" → ejecuta \`nextroute list-combos\`
- "CLI set-budget 10" → ejecuta \`nextroute set-budget 10\`

### Conocimiento
- "Cómo funciona NextRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with NextRoute.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runNextRouteCli") {
    const tool = getCopilotTool("runNextRouteCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runNextRouteCli", args: intent.args, result }],
    };
  }

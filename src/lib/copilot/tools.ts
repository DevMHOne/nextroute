/**
 * NextRoute Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure NextRoute on behalf of the user,
...
// ── Helper: check if nextroute CLI is available ──────────────────────────────

function getNextRouteCliPath(): string | null {
  try {
    const result = execSync("which nextroute 2>/dev/null || command -v nextroute 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the NextRoute codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within NextRoute.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runNextRouteCli",
    description:
      "Execute an 'nextroute' CLI command to configure or query the NextRoute app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: nextroute list-keys, nextroute switch-combo [id], nextroute set-budget 10, nextroute set-strategy [id] priority, nextroute health, nextroute mcp (starts MCP server), nextroute db-health, nextroute reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'nextroute'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getNextRouteCliPath();
      if (!cliPath) return "nextroute CLI not found in PATH. Install NextRoute first.";

      try {
        const output = execSync(`nextroute ${cmd}`, {
          encoding: "utf-8",
          timeout: 30000,
          maxBuffer: 1024 * 1024,
        });

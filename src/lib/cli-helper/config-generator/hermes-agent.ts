 ...
 *   model: string;                    // the model name the user chose from NextRoute
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the NextRoute base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_NEXTROUTE_API_KEY_HERE";
  ...
  // Build the providers.nextroute entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const nextrouteProvider = {

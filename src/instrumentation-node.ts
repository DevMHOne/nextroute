...
function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.NEXTROUTE_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  ...
  await import("@nextroute/open-sse/index.ts");
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@nextroute/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@nextroute/open-sse/services/systemPrompt.ts");

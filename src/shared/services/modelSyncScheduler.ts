...
const globalState = globalThis as typeof globalThis & {
  __nextrouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__nextrouteModelSyncInternalAuthToken || randomUUID();
    globalState.__nextrouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__nextrouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__nextrouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call NextRoute's own API

 ...
 *   - forwarding to the NextRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@nextroute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@nextroute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the NextRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the NextRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-nextroute-source`, `x-nextroute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.NEXTROUTE_BASE_URL ?? "http://127.0.0.1:20128";
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-nextroute-agent": this.agentId,

...
<!-- skill:custom-start -->
<!-- Migrated from skills/nextroute-monitoring/SKILL.md (preserved curated content) -->

# NextRoute — Monitoring & Health
...
```bash
curl $NEXTROUTE_URL/api/monitoring/health \
  -H "Authorization: Bearer $NEXTROUTE_KEY"
```
...
```bash
curl $NEXTROUTE_URL/api/providers/metrics \
  -H "Authorization: Bearer $NEXTROUTE_KEY"
```
...
## Via MCP (if NextRoute is your MCP server)

```
nextroute_get_health            → full system health snapshot
nextroute_get_provider_metrics  → p50/p95/p99 + circuit state per provider
nextroute_get_session_snapshot  → cost, tokens, errors for current session
nextroute_check_quota           → quota balance + percent remaining + reset time
nextroute_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $NEXTROUTE_URL/api/quota \
  -H "Authorization: Bearer $NEXTROUTE_KEY"
```
...
```bash
curl -X POST $NEXTROUTE_URL/api/budget/guard \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
...
NextRoute logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$NEXTROUTE_URL/api/mcp/status" \
  -H "Authorization: Bearer $NEXTROUTE_KEY"
```
...
- `503` on health endpoint → NextRoute is starting up; retry in 5s

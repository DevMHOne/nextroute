...
<!-- skill:custom-start -->
<!-- Migrated from skills/nextroute-routing/SKILL.md (preserved curated content) -->

# NextRoute — Routing & Combos
...
```bash
curl -X POST $NEXTROUTE_URL/api/combos \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "my-combo",
...
```bash
# Activate
curl -X PUT $NEXTROUTE_URL/api/combos/{id}/toggle \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
  -d '{ "enabled": true }'
```
...
```bash
curl $NEXTROUTE_URL/api/combos/{id}/metrics \
  -H "Authorization: Bearer $NEXTROUTE_KEY"
```
...
```bash
curl -X POST $NEXTROUTE_URL/api/routing/simulate \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
  -H "Content-Type: application/json" \
...
```
nextroute_list_combos     → list all combos
nextroute_switch_combo    → enable/disable a combo
nextroute_set_routing_strategy → change strategy at runtime
nextroute_simulate_route  → dry-run routing decision
nextroute_best_combo_for_task → get recommendation by task type
```

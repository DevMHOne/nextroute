...
<!-- skill:custom-start -->
<!-- Migrated from skills/nextroute-compression/SKILL.md (preserved curated content) -->
...
NextRoute compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
```bash
curl $NEXTROUTE_URL/api/settings/compression \
...
```bash
curl -X PUT $NEXTROUTE_URL/api/settings/compression \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST $NEXTROUTE_URL/api/compression/preview \
  -H "Authorization: Bearer $NEXTROUTE_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "rtk",
...
When NextRoute is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
nextroute_compression_status     → current settings + savings analytics
nextroute_compression_configure  → update mode/threshold/language
nextroute_set_compression_engine → switch engine at runtime
```

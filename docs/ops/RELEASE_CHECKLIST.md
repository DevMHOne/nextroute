...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `NEXTROUTE_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.nextroute/nextroute.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/nextroute/app/`.
...
- [ ] `nextroute --tray` boots on macOS (systray2 installed into `~/.nextroute/runtime/`)
- [ ] `nextroute --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `nextroute --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `nextroute config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g nextroute@<this-version>` runs postinstall without fatal exit
- [ ] `nextroute status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/nextroute*/SKILL.md` files are publicly fetchable via raw GitHub URL

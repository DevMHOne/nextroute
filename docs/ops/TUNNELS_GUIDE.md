...
NextRoute can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — NextRoute starts/stops the underlying
...
- **Auto-install.** On first use, NextRoute downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  NextRoute. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, NextRoute uses it instead of downloading. |
...
must be configured in the ngrok dashboard — NextRoute itself only forwards the
...
1. Install Tailscale (or let NextRoute do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via NextRoute's `login` endpoint).
...
NextRoute's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose NextRoute through a tunnel, the dashboard and OAuth flows must
...
and restart NextRoute before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; NextRoute will track each
independently.
...
NextRoute attempts to auto-install on first use. If the install is blocked

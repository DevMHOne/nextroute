---
title: "NextRoute — Deployment Guide on VM with Cloudflare"
version: 3.8.2
lastUpdated: 2026-05-13
---

# NextRoute — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure NextRoute on a VM (VPS) with domain managed via Cloudflare.
...
```bash
mkdir -p /opt/nextroute
```

### 2.2 Create environment variables file

```bash
cat > /opt/nextroute/.env << 'EOF'
# === Security ===
...
NEXTROUTE_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.nextroute.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.nextroute.online
EOF
```
...
```bash
docker pull diegosouzapw/nextroute:latest

docker run -d \
  --name nextroute \
  --restart unless-stopped \
  --env-file /opt/nextroute/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep nextroute
docker logs nextroute --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/nextroute << 'NGINX'
...
Keep reverse-proxy stream timeouts aligned with your NextRoute timeout env vars. If you raise
...
# Enable NextRoute
ln -sf /etc/nginx/sites-available/nextroute /etc/nginx/sites-enabled/nextroute
...
```bash
docker pull diegosouzapw/nextroute:latest
docker stop nextroute && docker rm nextroute
docker run -d --name nextroute --restart unless-stopped \
  --env-file /opt/nextroute/.env \
  -p 20128:20128 \
  -v nextroute-data:/app/data \
  diegosouzapw/nextroute:latest
...
```bash
docker logs -f nextroute          # Real-time stream
docker logs nextroute --tail 50   # Last 50 lines
```
...
docker cp nextroute:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v nextroute-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/nextroute-data-$(date +%F).tar.gz /data
```

### Restore from backup

```bash
docker stop nextroute
docker run --rm -v nextroute-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/nextroute-data-YYYY-MM-DD.tar.gz -C /"
docker start nextroute
```
...
See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `nextrouteCloud/` worker lives in a separate companion repo.
...
| 20128 | NextRoute   | Localhost only (via nginx) |

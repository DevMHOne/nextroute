...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against nextroute and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `nextroute@3.8.5` and
the mitigations applied in `nextroute@3.8.6`. It exists so:
...
Caddy. The fact that they exist in NextRoute is documented at
...
A `NEXTROUTE_ZED_IMPORT_LEGACY_ONE_STEP=true` env flag preserves the v3.8.5
...
who already use Zed and want to mirror their provider keys into NextRoute
without re-pasting.
...
toolkit." Within NextRoute they are only used by the MITM cert install path
...
(`NEXTROUTE_BUILD_PROFILE=minimal`) replaces
...
## §5 — NextRoute Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
1. **HMAC verification**: `verifyCloudSignature(rawBody, sigHeader)` checks
   the `X-Cloud-Sig` header (`HMAC-SHA256(NEXTROUTE_CLOUD_SYNC_SECRET,
...
2. **Secret-field opt-in**: `accessToken` / `refreshToken` /
   ...
   `NEXTROUTE_CLOUD_SYNC_SECRETS=true`. The default mode syncs only
...
**Why we keep it**: Cloud Sync is the only way for an NextRoute Cloud tenant
...
```bash
NEXTROUTE_BUILD_PROFILE=minimal npm run build
```
...
The resulting bundle is intended to be published as `nextroute-secure`. See

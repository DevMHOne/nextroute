...
For most users, **no special setup is required**. NextRoute ships a built-in
...
NextRoute also supports AgentRouter (and similar relays) through the **Claude Code
...
- NextRoute running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting NextRoute:
...
**`Invalid JSON response from provider (reset after Ns)` from the nextroute logs** —

...
Common problems and solutions for NextRoute.
...
| EACCES: permission denied                           | Set `DATA_DIR=/path/to/writable/dir` to override `~/.nextroute`                                                                                          |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Run `cd $(npm root -g)/nextroute/app && npm rebuild better-sqlite3 && nextroute` — see [macOS native module rebuild](#macos-native-module-rebuild) below |
...
**Cause:** You are running a Node.js version outside NextRoute's approved secure runtime floor. The most common case is running an older Node 20, 22, or 24 patch level that falls below the patched security floor NextRoute requires.
...
3. Reinstall NextRoute: `npm install -g nextroute`
4. Restart: `nextroute`
...
```
dlopen(/Users/<user>/.nvm/versions/node/v24.14.1/lib/node_modules/nextroute/app/node_modules/better-sqlite3/build/Release/better_sqlite3.node, 0x0001): tried: '...' (slice is not valid mach-o file)
```

**Fix — rebuild for your local environment (no Node.js downgrade required):**

```bash
cd $(npm root -g)/nextroute/app
...
**Fix (v3.5.5+):** NextRoute now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.
...
NextRoute auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://nextroute.dev`)
...
When many concurrent requests hit a rate-limited provider, NextRoute uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some NextRoute users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: NextRoute looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in NextRoute
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the NextRoute logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind NextRoute.
...
2. Restart NextRoute so the new env values are picked up
...
3. Restart NextRoute and re-test from **Dashboard → CLI Tools**
...
- Run `nextroute providers` from the CLI to re-trigger the OAuth flow, or
...
### NEXTROUTE_WS_BRIDGE_SECRET missing in production
...
**Cause:** The `NEXTROUTE_WS_BRIDGE_SECRET` env var is missing from the production environment.

**Fix:**

1. Generate a random secret: `openssl rand -hex 32`
2. Set `NEXTROUTE_WS_BRIDGE_SECRET=<random-secret>` in the production server env (and any client that talks to the bridge)
3. Restart NextRoute
...
- **GitHub Issues**: [github.com/diegosouzapw/NextRoute/issues](https://github.com/diegosouzapw/NextRoute/issues)

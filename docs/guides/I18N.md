...
NextRoute supports **30 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
NextRoute uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through NextRoute
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `NEXTROUTE_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `NEXTROUTE_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `NEXTROUTE_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__NEXTROUTE_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including NextRoute itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `nextroute` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `nextroute --lang de status`            |
| 2        | `NEXTROUTE_LANG` env var | `NEXTROUTE_LANG=ja nextroute providers` |
...
```bash
# Set language and save to ~/.nextroute/.env (persists across sessions)
nextroute config lang set pt-BR

# View current language
nextroute config lang get

# List all 42 available languages
nextroute config lang list

# JSON output
nextroute config lang list --output json
```

The saved preference is written atomically to `~/.nextroute/.env` and is loaded by the
...
nextroute --lang de providers list
```

...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to NextRoute,
...
## How NextRoute Solves This (v3.8.0+)

Starting with v3.8.0, NextRoute calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each NextRoute connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; NextRoute reads credentials from the
     local kiro-cli database or `~/.aws/sso/cache`.
3. The connection is saved. NextRoute automatically registers a dedicated OIDC client for it.
...
(`clientSecretExpiresAt`). NextRoute stores this timestamp in `providerSpecificData`
...
- Ensure NextRoute can reach `https://oidc.us-east-1.amazonaws.com` (or the configured

# NextRoute MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `nextroute_get_health`          | Gateway health, circuit breakers, uptime |
| `nextroute_list_combos`         | All configured combos with models        |
| `nextroute_get_combo_metrics`   | Performance metrics for a specific combo |
| `nextroute_switch_combo`        | Switch active combo by ID/name           |
| `nextroute_check_quota`         | Quota status per provider or all         |
| `nextroute_route_request`       | Send a chat completion through NextRoute |
| `nextroute_cost_report`         | Cost analytics for a time period         |
| `nextroute_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `nextroute_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `nextroute_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `nextroute_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `nextroute_test_combo`             | Live-test all models in a combo via a real upstream request |
| `nextroute_get_provider_metrics`   | Detailed metrics for one provider                           |
| `nextroute_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `nextroute_explain_route`          | Explain a past routing decision                             |
| `nextroute_get_session_snapshot`   | Full session state: costs, tokens, errors                   |

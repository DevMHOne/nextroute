# CLI Tools Setup Guide — NextRoute (Português (Portugal))
...
to use **NextRoute** as the unified backend, giving you centralized key management,
...
| **Antigravity**    | `antigravity` | internal   | mitm       | NextRoute      |
...
1. Open the NextRoute dashboard → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-nextroute-key"
export GEMINI_BASE_URL="http://localhost:20128/v1"
export GEMINI_API_KEY="sk-your-nextroute-key"
```
...
mkdir -p ~/.cline/data && cat > ~/.cline/data/globalState.json << EOF
{
  ...
  "openAiApiKey": "sk-your-nextroute-key"
}
EOF
```
...
Or use the NextRoute dashboard → **CLI Tools → Cline → Apply Config**.
...
```bash
kilocode --api-base http://localhost:20128/v1 --api-key sk-your-nextroute-key
```
...
```json
{
  ...
  "kilo-code.apiKey": "sk-your-nextroute-key"
}
```

Or use the NextRoute dashboard → **CLI Tools → KiloCode → Apply Config**.
...
# The CLI uses its own auth — NextRoute is not needed as backend for Kiro CLI itself.
# Use kiro-cli alongside NextRoute for other tools.
kiro-cli status
```
...
OPENAI_API_KEY="sk-your-nextroute-key"
...
> **Note:** Cursor routes requests through its cloud. For NextRoute integration,
> enable **Cloud Endpoint** in NextRoute Settings and use your public domain URL.

Via GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: your NextRoute key

---

## Dashboard Auto-Configuration

The NextRoute dashboard automates configuration for most tools:
...
**Droid** and **OpenClaw** are AI agents built directly into NextRoute — no installation needed.
They run as internal routes and use NextRoute's model routing automatically.
...
| Error                     | Cause                   | Fix                                        |
| ------------------------- | ----------------------- | ------------------------------------------ |
| `Connection refused`      | NextRoute not running   | `pm2 start nextroute`                      |
...
```bash
# Install all CLIs and configure for NextRoute (replace with your key and server URL)
NEXTROUTE_URL="http://localhost:20128/v1"
NEXTROUTE_KEY="sk-your-nextroute-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$NEXTROUTE_URL\",\"apiKey\":\"$NEXTROUTE_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $NEXTROUTE_KEY\napiBaseUrl: $NEXTROUTE_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$NEXTROUTE_URL"
export OPENAI_API_KEY="$NEXTROUTE_KEY"
export ANTHROPIC_BASE_URL="$NEXTROUTE_URL"
export ANTHROPIC_API_KEY="$NEXTROUTE_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for NextRoute"
```

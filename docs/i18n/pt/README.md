# 🚀 NextRoute — The Free AI Gateway (Português (Portugal))
...
![NPM Downloads](https://img.shields.io/npm/dw/nextroute?label=npm%20down%20week&color=red)
![NPM Downloads](https://img.shields.io/npm/dm/nextroute?label=npm%20down%20month&color=red)

![NPM Downloads](https://img.shields.io/npm/d18m/nextroute?label=npm%20down%20year&color=red)
![Docker Pulls](https://img.shields.io/docker/pulls/diegosouzapw/nextroute)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/diegosouzapw/nextroute/total?style=flat&label=eletron%20donwloads&color=blue)

[![stars](https://custom-icon-badges.demolab.com/github/stars/diegosouzapw/NextRoute?logo=star&style=flat)](https://github.com/diegosouzapw/NextRoute/stargazers)
[![open issues](https://custom-icon-badges.demolab.com/github/issues-raw/diegosouzapw/NextRoute?logo=issue)](https://github.com/diegosouzapw/NextRoute/issues)
[![license](https://custom-icon-badges.demolab.com/github/license/diegosouzapw/NextRoute?logo=law)](https://github.com/diegosouzapw/NextRoute/blob/main/LICENSE)
[![last commit](https://custom-icon-badges.demolab.com/github/last-commit/diegosouzapw/NextRoute?logo=history&logoColor=white)](https://github.com/diegosouzapw/NextRoute/commits/main)
...
[![code size](https://custom-icon-badges.demolab.com/github/languages/code-size/diegosouzapw/NextRoute?logo=file-code&logoColor=white)](https://github.com/diegosouzapw/NextRoute)
[![pr closed](https://custom-icon-badges.demolab.com/github/issues-pr-closed/diegosouzapw/NextRoute?color=purple&logo=git-pull-request&logoColor=white)](https://github.com/diegosouzapw/NextRoute/pulls?q=is%3Apr+is%3Aclosed)
[![tag](https://custom-icon-badges.demolab.com/github/v/tag/diegosouzapw/NextRoute?logo=tag&logoColor=white)](https://github.com/diegosouzapw/NextRoute/tags)
...
[![fork](https://custom-icon-badges.demolab.com/github/forks/diegosouzapw/NextRoute?logo=fork)](https://github.com/diegosouzapw/NextRoute/network/members)
[![watch](https://custom-icon-badges.demolab.com/github/watchers/diegosouzapw/NextRoute?logo=eye)](https://github.com/diegosouzapw/NextRoute/watchers)

[![License](https://img.shields.io/github/license/diegosouzapw/NextRoute)](https://github.com/diegosouzapw/NextRoute/blob/main/LICENSE)
...
[🌐 Website](https://nextroute.online) • [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Docs](#-documentation) • [💰 Pricing](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
<div align="center">
  <img src="./docs/screenshots/MainNextRoute.png" alt="NextRoute Dashboard" width="800"/>
</div>
...
_Connect any AI-powered IDE or CLI tool through NextRoute — free API gateway for unlimited coding._
...
<sub>📡 All agents connect via <code>http://localhost:20128/v1</code> or <code>http://cloud.nextroute.online/v1</code> — one config, unlimited models and quota</sub>
...
- **Issues**: [github.com/diegosouzapw/NextRoute/issues](https://github.com/diegosouzapw/NextRoute/issues)
...
This generates a `system-info.txt` with your Node.js version, NextRoute version, OS details, installed CLI tools (qoder, gemini, claude, codex, antigravity, droid, etc.), Docker/PM2 status, and system packages — everything we need to reproduce your issue quickly. Attach the file directly to your GitHub issue.
...
## 🎯 What NextRoute Solves — 30 Real Pain Points & Use Cases

> **Every developer using AI tools faces these problems daily.** NextRoute was built to solve them all — from cost overruns to regional blocks, from broken OAuth flows to protocol operations and enterprise observability.
...
- **System Info Report** — `npm run system-info` generates `system-info.txt` with your full environment (Node version, NextRoute version, OS, CLI tools, Docker/PM2 status). Attach it when reporting issues for instant triage.
...
- **npm global install** — `npm install -g nextroute && nextroute` — done
...
- Built-in quick-start for `nextroute --mcp` and client onboarding
...
```txt
1) Start MCP transport (`nextroute --mcp`) for tool-driven operations
...
Arch Linux users can install the [AUR package](https://aur.archlinux.org/packages/nextroute-bin), which installs NextRoute and provides a systemd user service:
...
When you no longer need NextRoute, we provide two quick scripts for a clean removal:
...
> Note: To run these commands, navigate to the NextRoute project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g nextroute`.
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), NextRoute also derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, NextRoute keeps the default
...
`cache_control`, NextRoute does not inject bridge-owned markers.
...
For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, NextRoute will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

If you run NextRoute behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy
timeouts are also higher than your NextRoute stream/fetch timeouts.
...
### 3) Point your coding tool to NextRoute
...
- `nextroute_get_health`
- `nextroute_list_combos`
...
NextRoute is available as a public Docker image on [Docker Hub](https://hub.docker.com/r/diegosouzapw/nextroute).
...
docker run -d \
  ...
  --stop-timeout 40 \
  -p 20128:20128 \
  -v nextroute-data:/app/data \
  diegosouzapw/nextroute:latest
```
...
- Quick Tunnels are not auto-restored after an NextRoute or container restart. Re-enable them from the dashboard when needed.
- Managed install currently supports Linux, macOS, and Windows on `x64` / `arm64`.
- Managed Quick Tunnels default to HTTP/2 transport to avoid noisy QUIC UDP buffer warnings in constrained container environments. Set `CLOUDFLARED_PROTOCOL=quic` or `auto` if you want a different transport.
- Docker images bundle system CA roots and pass them to managed `cloudflared`, which avoids TLS trust failures when the tunnel bootstraps inside the container.
- SQLite runs in WAL mode. `docker stop` should be allowed to finish so NextRoute can checkpoint the latest changes back into `storage.sqlite`.
...
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want NextRoute to use an existing binary instead of downloading one.

**Using Docker Compose with Caddy (HTTPS Auto-TLS):**

NextRoute can be securely exposed using Caddy's automatic SSL provisioning. Ensure your domain's DNS A record points to your server's IP.

```yaml
services:
  nextroute:
    image: diegosouzapw/nextroute:latest
    container_name: nextroute
    restart: unless-stopped
    volumes:
      - nextroute-data:/app/data
    environment:
      - PORT=20128
  ...
  caddy:
    ...
    ports:
      - "80:80"
      - "443:443"
    command: caddy reverse-proxy --from https://your-domain.com --to http://nextroute:20128
...
| ------------------------ | -------- | ------ | --------------------- |
| `diegosouzapw/nextroute` | `latest` | ~250MB | Latest stable release |
| `diegosouzapw/nextroute` | `3.6.2`  | ~250MB | Current version       |

---

## 🖥️ Desktop App — Offline & Always-On

> 🆕 **NEW!** NextRoute is now available as a **native desktop application** for Windows, macOS, and Linux.

Run NextRoute as a standalone desktop app — no terminal, no browser, no internet required for local models. The Electron-based app includes:

- 🖥️ **Native Window** — Dedicated app window with system tray integration
- 🔄 **Auto-Start** — Launch NextRoute on system login
...
When minimized, NextRoute lives in your system tray with quick actions:
...
**Zero cost. Never stops coding.** Configure this as one NextRoute combo and all fallbacks happen automatically — no manual switching ever.
...
> All models below are **100% free with zero credit card required**. NextRoute auto-routes between them when one quota runs out — combine them all for an unbreakable $0 combo.
...
NextRoute v3.6 is built as an operational platform, not just a relay proxy.
...
When quota, rate, or health fails, NextRoute automatically moves to the next candidate without manual switching.
...
NextRoute includes a built-in evaluation framework to test LLM response quality against a golden set. Access it via **Analytics → Evals** in the dashboard.

### Built-in Golden Set

The pre-loaded "NextRoute Golden Set" contains test cases for:
...
2. Run `nextroute_get_health`.
3. Run `nextroute_list_combos`.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. NextRoute tracks quota per model!
...
- Rotation behavior: NextRoute routes to the next eligible Codex account automatically.
...
**Pro Tip:** OpenAI-compatible API — works seamlessly with NextRoute's format translation!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from NextRoute dashboard]
...
export OPENAI_API_KEY="your-nextroute-api-key"
...
> **Note:** OpenClaw only works with local NextRoute. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
Settings → API Configuration:
  ...
  Base URL: http://localhost:20128/v1
  API Key: [from NextRoute dashboard]
  Model: if/kimi-k2-thinking
```

### OpenCode

**Step 1:** Add NextRoute as a custom provider:

```bash
opencode
/connect
# Select "Other" → Enter ID: "nextroute" → Enter your NextRoute API key
```
...
```bash
/models
# Select any NextRoute model from the list
```

> **Tip:** Add any model available in your NextRoute `/v1/models` endpoint to the `models` section. Use the format `provider/model-id` from your NextRoute dashboard.
...
- NextRoute v1.0.6+ includes fallback validation via chat completions
...
> **⚠️ Important for users running NextRoute on a VPS, Docker, or any remote server**
...
The OAuth credentials bundled in NextRoute are registered **for `localhost` only**. When you access NextRoute on a remote server (e.g. `https://nextroute.myserver.com`), Google rejects the authentication with:
...
- Name: anything you like (e.g. `NextRoute Remote`)
...
1. NextRoute opens the Google authorization URL
...
4. Paste that URL into the field shown in the NextRoute connection modal
5. Click **"Connect"**
...
As credenciais OAuth embutidas no NextRoute estão cadastradas **apenas para `localhost`**. Quando você acessa o NextRoute em um servidor remoto (ex: `https://nextroute.meuservidor.com`), o Google rejeita a autenticação com:
...
- Nome: escolha qualquer nome (ex: `NextRoute Remote`)
...
1. O NextRoute abrirá a URL de autorização do Google
2. Após você autorizar, o Google tentará redirecionar para `localhost` (que falha no servidor remoto)
3. **Copie a URL completa** da barra de endereço do seu browser (mesmo que a página não carregue)
4. Cole essa URL no campo que aparece no modal de conexão do NextRoute
5. Clique em **"Connect"**
...
NextRoute has **218+ features planned** across multiple development phases. Here are the key areas:
...
<div align="center">
  ...
  <br/>
  <sub><a href="https://nextroute.online">nextroute.online</a></sub>
</div>

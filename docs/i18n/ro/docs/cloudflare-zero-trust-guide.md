...
Este guia documenta o padrão ouro de infraestrutura de rede para proteger o **NextRoute** e expor sua aplicação de forma segura para a internet, **sem abrir nenhuma porta (Zero Inbound)**.

## O que foi feito na sua VM?

Nós ativamos o NextRoute em modo **Split-Port** através do PM2:
...
3. Clique em **Add a Tunnel**, escolha **Cloudflared** e dê o nome \`NextRoute-VM\`.
...
3. Em **Application name**, coloque \`Painel NextRoute\`.
4. Em **Application domain**, coloque \`nextroute.seuglobal.com.br\` (O mesmo que você fez na "Rota 2").
5. Clique em **Next**.
...
4. **Name:** \`Anti-Abuso NextRoute API\`
...
2. O NextRoute só conversa HTTPS saindo (\`cloudflared\`) e não recebendo TCP direto do mundo.

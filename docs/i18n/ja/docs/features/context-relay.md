...
- before the active account is exhausted, NextRoute generates a compact structured summary
- after authentication selects a different account for the same session, NextRoute injects
...
If the active provider is enabled in `handoffProviders`, NextRoute generates a structured
...
NextRoute prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, NextRoute converts that payload into a `<context_handoff>` system

...
**NextRoute** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
NextRoute har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@nextroute/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@nextroute/open-sse` → `open-sse/`, `@nextroute/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.nextroute/`

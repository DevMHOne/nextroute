...
**NextRoute** — einheitlicher KI-Proxy/Router. Ein Endpunkt, 160+ LLM-Anbieter, automatischer Fallback.
...
NextRoute hat drei verwandte, aber unterschiedliche Mechanismen für temporäre Fehler. Halten Sie ihren
...
- **Imports**: extern → intern (`@/`, `@nextroute/open-sse`) → relativ
...
- **Pfad-Aliase**: `@/*` → `src/`, `@nextroute/open-sse` → `open-sse/`, `@nextroute/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + Dashboard am selben Port)
- **Datenverzeichnis**: `DATA_DIR` Umgebungsvariable, standardmäßig `~/.nextroute/`

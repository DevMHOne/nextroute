---
title: "NextRoute v3.8.0 Release Notes"
version: 3.8.0
releaseDate: 2026-05-15
---

# NextRoute v3.8.0 — Release Notes
...
Ten `SKILL.md` manifests under `skills/nextroute*/` published for external AI
...
`systray2` fork installed lazily into `~/.nextroute/runtime/`. Includes
cross-platform autostart (LaunchAgent / `.desktop` / registry).

- New flag: `nextroute --tray`
- New command: `nextroute config tray <enable|disable>`
- New modules: `bin/cli/tray/`

### CLI machine-ID token (Task 4)

`HMAC-SHA256(machine-id, salt)` → constant-time-compared token sent via
`x-nextroute-cli-token` header. Lets CLI commands authenticate locally
without JWT/password.

- Accepted only on loopback host + whitelisted read-only routes
- Salt rotation via `NEXTROUTE_CLI_SALT` env var
- Falls back to `~/.nextroute/machine-id` (0600) if `node-machine-id` fails
...
load. Solves Windows EBUSY error on global updates while NextRoute is running.

- New scripts: `scripts/postinstall.mjs`, `bin/cli/runtime/`
- Runtime dir: `~/.nextroute/runtime/`
- New helper: `src/lib/db/core.ts::ensureDbInitialized()`
- Docs: `docs/ops/SQLITE_RUNTIME.md`

### Tier 1/2/3 marketing & onboarding (Task 8)

README "Why NextRoute?" section enhanced with ASCII tier diagram and comparison
...
```bash
npm install -g nextroute@3.8.0
nextroute setup   # optional — re-runs onboarding to show new tier tour
```

CLI tokens are derived automatically; no migration needed.
To enable tray: `nextroute --tray` or `nextroute config tray enable`.

For developers embedding NextRoute:

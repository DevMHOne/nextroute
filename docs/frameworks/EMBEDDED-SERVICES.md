...
Embedded services are locally-installed process sidecar tools that NextRoute installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as NextRoute and communicate over loopback.
...
| --------------- | ---------------------------------------------- | :----------: | ---------------------------------------------------------------------------------------------------- |
| **9Router**     | `9router`                                      |    20130     | AI router that NextRoute can use as a sub-provider. Models exposed as `9router/{sub}/{model}`        |
...
- NextRoute installs them under `DATA_DIR/services/{name}/` (isolated from NextRoute's own `package.json`)
- NextRoute spawns and monitors them as child processes
- NextRoute injects an ephemeral API key into the child's environment and rotates it without downtime
...
| API key management                    | NextRoute generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with NextRoute's own deps.
...
the next time NextRoute boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of NextRoute session)
...
9Router requires an API key for its own HTTP endpoints. NextRoute:
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs NextRoute.
...
NextRoute's default port 20128.
...
- `DATA_DIR` or its parent is not writable by the NextRoute process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.nextroute/`): `ls -la ~/.nextroute/`
2. Ensure the NextRoute process user owns the directory: `chown -R $USER ~/.nextroute/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.nextroute/services/{name}/`.
...
Yes. Both services follow the same local-first model as NextRoute itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where NextRoute is deployed, not a remote cloud provider.
...
2. Check structured logs in NextRoute's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.nextroute/nextroute.db "SELECT * FROM version_manager WHERE tool='9router'"`.

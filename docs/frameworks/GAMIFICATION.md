...
NextRoute includes a local-first gamification layer that rewards users for
...
| Community Servers | Federate with external NextRoute instances                      |
...
All tables live in the main NextRoute SQLite database, created by migration
...
Follows the standard NextRoute pattern — imports `getDbInstance()` from
...
The raw token is returned to the user exactly once at creation time. NextRoute
...
All routes follow the standard NextRoute pattern:

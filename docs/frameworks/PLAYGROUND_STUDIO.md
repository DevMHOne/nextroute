...
from the current `PlaygroundState`. API key placeholder is always `$NEXTROUTE_API_KEY` (D11).
...
| Export code shows `MISSING_API_KEY` | Placeholder not inserted | `codeExport.ts` always uses `API_KEY_PLACEHOLDER = "$NEXTROUTE_API_KEY"` |

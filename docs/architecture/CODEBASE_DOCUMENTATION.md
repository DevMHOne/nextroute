---
title: "NextRoute Codebase Documentation"
version: 3.8.2
lastUpdated: 2026-05-13
---

# NextRoute Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-05-13
> **Audience:** Engineers contributing to NextRoute or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@nextroute/open-sse` → `open-sse/index.ts`
- `@nextroute/open-sse/*` → `open-sse/*`

Default HTTP port: **`20128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.nextroute/`.
...
├── open-sse/             Streaming engine workspace (@nextroute/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (nextroute, reset-password)
...
| `responses.ts`, `nextrouteResponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@nextroute/open-sse`. Owns request
...
```
bin/
├── nextroute.mjs           Main CLI entry (Node ESM)
...
- `nextroute` → `bin/nextroute.mjs`
- `nextroute-reset-password` → `bin/reset-password.mjs`
...
- **Imports**: external → internal (`@/`, `@nextroute/open-sse`) → relative.

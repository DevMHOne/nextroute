---
title: "NextRoute Architecture"
version: 3.8.2
lastUpdated: 2026-05-13
---

# NextRoute Architecture
...
NextRoute is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[NextRoute Local Process]
...
NextRoute can install, supervise, and route to locally-running AI tool processes
...
- NextRoute response metadata builder: `src/domain/nextrouteResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/nextroute/storage.sqlite` when set, else `~/.nextroute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[NextRoute Runtime]
        Next[Next.js Server\nPORT=20128]
...
> NextRoute v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by NextRoute; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/nextroute` -> `~/.nextroute`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@nextroute/open-sse` **npm workspace package**. Source code imports it via `@nextroute/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t nextroute .`

...
**Problem it solves:** NextRoute proxies many API keys against the same upstream
...
- All persistence is in the existing NextRoute SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("nextroute:quota-share:pools")` is non-empty.

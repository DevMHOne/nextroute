#!/usr/bin/env node
/**
 * NextRoute — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   NEXTROUTE_TRANSLATION_API_URL     e.g. https://cloud.nextroute.dev/v1
 *   NEXTROUTE_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   NEXTROUTE_TRANSLATION_MODEL       e.g. cx/gpt-5.4-mini
 *   NEXTROUTE_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   NEXTROUTE_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("NEXTROUTE_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("NEXTROUTE_TRANSLATION_API_KEY");
  const model = requireEnv("NEXTROUTE_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.NEXTROUTE_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.NEXTROUTE_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.NEXTROUTE_TRANSLATION_API_URL || "").replace(/\/$/, "");

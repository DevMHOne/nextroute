#!/usr/bin/env node
/**
 * NextRoute — UI i18n key sync (next-intl message catalogs).
 *
 ...
 *
 * --translate-markers calls the NextRoute translation backend (same env vars
...
async function processLocale(locale, source, config, opts, backend) {
  ...
  if (opts.translateMarkers && placeholderCountBefore > 0 && backend) {
    ...
    if (!localeEntry) {
      ...
      const concurrency =
        opts.concurrency ?? Number(process.env.NEXTROUTE_TRANSLATION_CONCURRENCY || 4);

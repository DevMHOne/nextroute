...
async function runAutomatedChecks() {
  ...
  for (const code of legacyCheckLocales) {
    ...
    try {
      ...
      if (content.includes("**[English](#-nextroute--the-free-ai-gateway)**")) {
        anchorLineRemoved = false;
      }
      if (content.includes("## 🇧🇷 NextRoute")) {
...
async function main() {
  ...
  automatedChecksLines.push(
    ...
    `- Apêndice "## 🇧🇷 NextRoute" removido em ja/ar: **${automated.brAppendixRemoved ? "OK" : "PENDENTE"}**`,

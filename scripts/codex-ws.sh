#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um NextRoute LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O NextRoute expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O NextRoute, no
...
#   NEXTROUTE_WS_BASE   base URL do NextRoute local   (default abaixo)
#   NEXTROUTE_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   NEXTROUTE_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
NEXTROUTE_WS_BASE="${NEXTROUTE_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do NextRoute local
NEXTROUTE_WS_MODEL="${NEXTROUTE_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export NEXTROUTE_LOCAL_KEY="${NEXTROUTE_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do NextRoute.
model = "$NEXTROUTE_WS_MODEL"
model_provider = "nextroute-local"

[model_providers.nextroute-local]
name = "NextRoute Local (WS)"
base_url = "$NEXTROUTE_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "NEXTROUTE_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$NEXTROUTE_WS_MODEL" -c model_provider="nextroute-local")

#!/usr/bin/env node
/**
 * NextRoute — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.nextroute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by NextRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/nextroute.mjs`.
  ...
  if (needsPersist) {
    merged.NEXTROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "nextroute-field-encryption-v1", 32)
              const legacySalt = "nextroute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.nextroute/server.env or ~/.nextroute/.env,"
                );
                log(
                  "   or run: nextroute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }

#!/usr/bin/env node
/**
 * system-info.mjs — NextRoute System Information Reporter (#280)
 *
...
lines.push("NextRoute System Information Report");
...
const installedGlobal = run("npm list -g nextroute --depth=0 2>/dev/null | grep nextroute");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep nextroute | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/diegosouzapw/NextRoute/issues`
);

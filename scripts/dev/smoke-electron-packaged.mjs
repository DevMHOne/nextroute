...
export const LINUX_EXECUTABLE_NAMES = ["nextroute-desktop", "nextroute", "NextRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "NextRoute.app", "Contents", "MacOS", "NextRoute"),
    join(distDir, "mac-arm64", "NextRoute.app", "Contents", "MacOS", "NextRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "NextRoute.exe"),
    join(distDir, "win-x64-unpacked", "NextRoute.exe"),
    join(distDir, "win-arm64-unpacked", "NextRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["NextRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing NextRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["nextroute-desktop", "NextRoute", "nextroute"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged NextRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "nextroute-electron-smoke-")));
  const removeDataDir =

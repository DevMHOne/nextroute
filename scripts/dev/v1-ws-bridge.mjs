...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://nextroute.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://nextroute.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-nextroute-ws-session-id": this.sessionId,
      "x-nextroute-ws-request-id": requestId,
    };
...
export function createNextrouteWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createNextrouteWsBridge requires a baseUrl");
  }

...
import { createNextrouteWsBridge } from "./v1-ws-bridge.mjs";
...
if (fs.existsSync(rootAppDir) && fs.statSync(rootAppDir).isDirectory()) {
  ...
  console.error("Please rename or delete the root 'app/' directory before starting NextRoute.\n");
  process.exit(1);
}
...
const useTurbopack = dev && mergedEnv.NEXTROUTE_USE_TURBOPACK === "1";
...
async function start() {
  ...
  const wsBridge = createNextrouteWsBridge({
    baseUrl: `http://127.0.0.1:${dashboardPort}`,
  });

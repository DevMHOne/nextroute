# bin/cli — NextRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `nextroute` binary.
...
├── sqlite.mjs              ← openNextRouteDb() — DB bootstrap
...
All HTTP calls to the NextRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `NEXTROUTE_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `NEXTROUTE_API_KEY`)
...
Locale detection order: `NEXTROUTE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `NEXTROUTE_LANG` environment variable
...
```bash
nextroute config lang set pt-BR       # saves to ~/.nextroute/.env
nextroute config lang list            # show all 42 available locales
nextroute config lang get             # show currently active locale
```

**One-time override:**

```bash
nextroute --lang de providers list    # run in German, not persisted
NEXTROUTE_LANG=ja nextroute status    # same effect via env
```

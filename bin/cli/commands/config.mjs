...
function ensureBackup(configPath) {
  ...
  const backupDir = path.join(path.dirname(configPath), ".nextroute.bak");
...
async function runConfigGetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: nextroute config get <tool>");
    return 1;
  }
...
async function runConfigSetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: nextroute config set <tool> [options]");
    return 1;
  }
  ...
  if (!apiKey) {
    printError("API key required. Use --api-key or set NEXTROUTE_API_KEY.");
    return 1;
  }
...
async function runConfigValidateCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: nextroute config validate <tool>");
    return 1;
  }
...
export async function runConfigLangSetCommand(code, opts = {}) {
  ...
  upsertEnvLine(envPath, "NEXTROUTE_LANG", code);
  setLocale(code);
  ...
  config
    ...
    .action(async (tool, opts, cmd) => {
      ...
      const exitCode = await runConfigSetCommand(tool, {
        ...opts,
        apiKey: opts.apiKey || globalOpts.apiKey || process.env.NEXTROUTE_API_KEY,
        baseUrl: opts.baseUrl || globalOpts.baseUrl || process.env.NEXTROUTE_BASE_URL,

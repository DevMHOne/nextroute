...
export async function runCompressionStatus(opts, cmd) {
  const data = await mcpCall("nextroute_compression_status", {}, restCompressionStatus);
...
export async function runCompressionConfigure(opts, cmd) {
  ...
  const data = await mcpCall("nextroute_compression_configure", config, () =>
...
export async function runCompressionEngineSet(name, opts, cmd) {
  ...
  await mcpCall("nextroute_set_compression_engine", { engine: normalized }, () =>
...
export function registerCompression(program) {
  ...
  combos.command("list").action(async (opts, cmd) => {
    const data = await mcpCall("nextroute_list_compression_combos", {}, async () => ({

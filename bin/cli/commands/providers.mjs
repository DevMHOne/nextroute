...
import { openNextRouteDb } from "../sqlite.mjs";
...
export async function runAvailableCommand(opts = {}) {
  ...
  if (opts.json) {
    ...
    printHeading("NextRoute Available Providers");
...
export async function runTestAllCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("NextRoute Provider Tests");
...
export async function runValidateCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("NextRoute Provider Validation");
      if (results.length === 0) {
        console.log("No providers configured.");
      }
      for (const result of results) {
...
export async function runProvidersRotateCommand(selector, opts = {}) {
  ...
  if (serverUp) {
    try {
      ...
      const { db: db2 } = await openNextRouteDb();
      try {
  ...
  // --- Post-rotation test ---
  if (!opts.skipTest) {
    const { db: db3 } = await openNextRouteDb();
    try {
...
export function registerProviders(program) {
  ...
  providers
    .command("available")
    .description("Show available providers in the NextRoute catalog")

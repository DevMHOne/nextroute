...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_nextroute");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "nextroute.fish");
  return join(home, ".bash_completion.d", "nextroute");
}

function generateZshScript() {
  return `#compdef nextroute

# NextRoute zsh completion (dynamic)
_nextroute_get_cache() {
  local key="$1"
  local cache="$HOME/.nextroute/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    nextroute completion refresh --quiet >/dev/null 2>&1
  fi
...
_nextroute() {
  local -a commands
  commands=(
    'serve:Start the NextRoute server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure NextRoute'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_nextroute_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_nextroute_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_nextroute_get_cache models))
...
compdef _nextroute nextroute
`;
}

function generateBashScript() {
  return `#!/bin/bash
# NextRoute CLI bash completion (dynamic)
...
complete -F _nextroute nextroute
`;
}

function generateFishScript() {
  return `# NextRoute CLI fish completion (dynamic)
complete -c nextroute -f
...
for cmd in $commands
  complete -c nextroute -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c nextroute -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c nextroute -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c nextroute -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c nextroute -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c nextroute -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c nextroute -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __nextroute_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.nextroute/completion-cache.json"
...
complete -c nextroute -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__nextroute_cache_get combos)'
complete -c nextroute -l model -a '(__nextroute_cache_get models)'
complete -c nextroute -l combo -a '(__nextroute_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `nextroute completion <shell>` (positional arg form)
  comp
    .command("<shell>")

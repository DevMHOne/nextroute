...
const RUNTIME_DIR = join(homedir(), ".nextroute", "runtime");
...
export async function loadSqliteRuntime() {
  ...
  try {
    ...
    console.warn(`[nextroute] runtime install failed: ${err.message}`);
  }
  ...
  if (existsSync(buildDir)) {
    ...
    if (nodeFile) {
      ...
      if (!magic || (magic !== expected && magic !== "macho-le" && magic !== "macho-fat")) {
        console.warn(
          `[nextroute] runtime sqlite binary magic mismatch (${magic} ≠ ${expected}) — skipping`
        );
        return null;
      }
    }
...
function ensureRuntimeDir() {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(
      pkg,
      JSON.stringify({ name: "nextroute-runtime", private: true, type: "commonjs" }),
      "utf-8"
    );
  }
}

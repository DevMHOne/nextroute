# NextRoute CLI — Internal Conventions
...
```
nextroute keys add openai sk-xxx
nextroute combo switch fastest
nextroute memory search "react hooks"
```

**Not allowed**:

```
nextroute --add-key openai sk-xxx     # ❌ flag-as-verb
nextroute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `NEXTROUTE_BASE_URL` env or `~/.nextroute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${NEXTROUTE_API_KEY}` when available.
- Injects `x-nextroute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `NEXTROUTE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `NEXTROUTE_LANG` to `~/.nextroute/.env`.

 ...
 *
 * Purely informational invocations must NOT create `~/.nextroute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `nextroute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `nextroute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `nextroute` runs the default `serve` command → must provision.

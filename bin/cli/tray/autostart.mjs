...
const APP_LABEL = "com.nextroute.autostart";
const WIN_REG_VALUE = "NextRoute";
const LINUX_SERVICE_NAME = "nextroute.service";
const LINUX_DESKTOP_NAME = "nextroute.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v nextroute 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "nextroute.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("nextroute.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".nextroute", ".env");
  const lines = [
    "[Unit]",
    "Description=NextRoute AI proxy router",

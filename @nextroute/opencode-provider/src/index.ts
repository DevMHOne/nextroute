/**
 * OpenCode provider plugin for NextRoute AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * NextRoute instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any NextRoute-exposed
 ...
 *       apiKey: "sk_nextroute",
 *     });
 *     // config -> { $schema, provider: { nextroute: { npm, name, options, models } } }
 *     ```
 *
 ...
 *     const provider = createNextRouteProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.nextroute in opencode.json
 *     ```
 *
...
export const NEXTROUTE_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed NextRoute models. Synced
 * with the Alph4d0g/opencode-nextroute-auth NEXTROUTE_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-nextroute-auth, MIT) and extended
 * with Claude Code passthrough models (`cc/` prefix).
 */
export const NEXTROUTE_DEFAULT_OPENCODE_MODELS = [
  "cc/claude-opus-4-8",
  "cc/claude-opus-4-7",
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-nextroute-auth
 * (https://github.com/Alph4d0g/opencode-nextroute-auth, MIT).
 */
 ...
 * Matches the context lengths used by NextRoute's provider registry.
 */
export const NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `NextRouteProviderOptions.modelCapabilities`.
 */
export const NEXTROUTE_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface NextRouteProviderOptions {
  /** NextRoute base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** NextRoute API key. Required. Use `sk_nextroute` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"NextRoute"`. */
  displayName?: string;
  /** Override the model catalog. Defaults to `NEXTROUTE_DEFAULT_OPENCODE_MODELS`. */
   ...
   * `NEXTROUTE_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Primary model for OpenCode (top-level `model` key).
   * Emitted as `"nextroute/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.nextroute.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to NextRoute. */
  npm: typeof NEXTROUTE_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"nextroute/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"nextroute/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [NEXTROUTE_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@nextroute/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@nextroute/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@nextroute/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.nextroute` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createNextRouteProvider(options: NextRouteProviderOptions): OpenCodeProviderEntry {
  ...
  const modelList =
    options.models && options.models.length > 0
      ? [...options.models]
      : [...NEXTROUTE_DEFAULT_OPENCODE_MODELS];
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = NEXTROUTE_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const contextLength = NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
  ...
  return {
    npm: NEXTROUTE_PROVIDER_NPM,
    name: options.displayName?.trim() || "NextRoute",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.nextroute`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"nextroute/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildNextRouteOpenCodeConfig(
  options: NextRouteProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [NEXTROUTE_PROVIDER_KEY]: createNextRouteProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${NEXTROUTE_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${NEXTROUTE_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the NextRoute provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.nextroute` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildNextRouteOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [NEXTROUTE_PROVIDER_KEY]: partial.provider[NEXTROUTE_PROVIDER_KEY],
  };
 ...
 */
export const NEXTROUTE_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type NextRouteMCPScope = (typeof NEXTROUTE_MCP_DEFAULT_SCOPES)[number] | string;

export interface NextRouteMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** NextRoute API key forwarded to the MCP server as `NEXTROUTE_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `NEXTROUTE_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `NEXTROUTE_MCP_SCOPES`.
   * When omitted `NEXTROUTE_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: NextRouteMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.nextroute` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createNextRouteMCPEntry({
 *   serverPath: "/home/user/.local/share/nextroute/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_nextroute",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.nextroute
 * ```
 */
export function createNextRouteMCPEntry(options: NextRouteMCPOptions): OpenCodeMCPServerEntry {
  ...
  const env: Record<string, string> = {
    NEXTROUTE_API_KEY: apiKey,
  };

  if (options.managementApiKey !== undefined) {
    const mgmtKey = options.managementApiKey.trim();
    if (mgmtKey) env.NEXTROUTE_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.NEXTROUTE_MCP_ENFORCE_SCOPES = "true";
    env.NEXTROUTE_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@nextroute/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that NextRoute's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different NextRoute releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-nextroute-auth (MIT).
 */
export interface NextRouteLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running NextRoute instance.
 *
 ...
 * `display_name`) field variants across NextRoute versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createNextRouteProvider` / `buildNextRouteOpenCodeConfig` instead of
 * relying on `NEXTROUTE_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - NextRoute base URL (with or without `/v1`).
 * @param apiKey    - NextRoute API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_nextroute");
 * const config = buildNextRouteOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_nextroute",
...
export async function fetchLiveModels(
  ...
  const models: NextRouteLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from NextRoute's /v1/models response.
    // NextRoute returns context_length in snake_case for both synced
 ...
 */
export type NextRouteCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface NextRouteCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: NextRouteCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running NextRoute instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as NextRouteCompressionOverride)
      : "";
...
/**
 * Options for `createNextRouteComboConfig`.
 * Mirrors the subset of combo fields exposed by the NextRoute `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface NextRouteComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for NextRoute's management API.
 *
 ...
 * @example
 * ```ts
 * const payload = createNextRouteComboConfig({
 *   name: "claude-primary",
 *   strategy: "priority",
 ...
 * ```
 */
export function createNextRouteComboConfig(
  options: NextRouteComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface NextRouteRoleOverrides {
...
/** Per-role binding used by `createNextRouteAgentBlock`. */
export interface NextRouteAgentRole extends NextRouteRoleOverrides {
  /** NextRoute model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createNextRouteAgentBlock`. */
export interface NextRouteAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, NextRouteAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends NextRouteRoleOverrides {
  /** Always emitted as `"nextroute/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: NextRouteAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${NEXTROUTE_PROVIDER_KEY}/${modelId}` };
 ...
 * specific NextRoute model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createNextRouteAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "nextroute/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createNextRouteAgentBlock(
  options: NextRouteAgentBlockOptions
...
/**
 * Per-mode binding used by `createNextRouteModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `NextRouteAgentRole` + `createNextRouteAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface NextRouteMode extends NextRouteAgentRole {}

/**
 * Options for `createNextRouteModesBlock`.
 *
 * @deprecated See `NextRouteMode`.
 */
export interface NextRouteModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, NextRouteMode>;
}
 ...
 * a specific NextRoute model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createNextRouteAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createNextRouteModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createNextRouteModesBlock(
  options: NextRouteModesBlockOptions
...
export default createNextRouteProvider;

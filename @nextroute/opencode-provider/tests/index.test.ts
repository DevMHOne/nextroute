...
import {
  buildNextRouteOpenCodeConfig,
  createNextRouteAgentBlock,
  createNextRouteComboConfig,
  createNextRouteMCPEntry,
  createNextRouteModesBlock,
  createNextRouteProvider,
  fetchLiveModels,
  listCombos,
  mergeIntoExistingConfig,
  normalizeBaseURL,
  NEXTROUTE_DEFAULT_MODEL_CAPABILITIES,
  NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS,
  NEXTROUTE_DEFAULT_OPENCODE_MODELS,
  NEXTROUTE_MCP_DEFAULT_SCOPES,
  NEXTROUTE_PROVIDER_NPM,
...
test("createNextRouteProvider validates required fields", () => {
  assert.throws(
    () => createNextRouteProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createNextRouteProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createNextRouteProvider produces the OpenCode-compatible shape", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });

  assert.equal(provider.npm, NEXTROUTE_PROVIDER_NPM);
  assert.equal(provider.name, "NextRoute");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_nextroute");
  assert.equal(typeof provider.models, "object");
});

test("createNextRouteProvider seeds the default model catalog", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });
  ...
  const defaultIds = [...NEXTROUTE_DEFAULT_OPENCODE_MODELS].sort();
...
test("createNextRouteProvider honours a custom models list and labels", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
...
test("createNextRouteProvider deduplicates and trims model ids", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
...
test("createNextRouteProvider honours displayName override", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
    displayName: "Local NextRoute",
  });
  assert.equal(provider.name, "Local NextRoute");
});

test("buildNextRouteOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildNextRouteOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_nextroute",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider.nextroute, "object");
  assert.equal(doc.provider.nextroute.options.baseURL, "http://localhost:20128/v1");
});
...
test("buildNextRouteOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildNextRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  ...
  assert.equal(doc.model, "nextroute/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "nextroute/gemini-3-flash");
});

test("buildNextRouteOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildNextRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });
...
test("buildNextRouteOpenCodeConfig ignores blank model strings", () => {
  const doc = buildNextRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("nextroute" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing nextroute entry", () => {
  const existing = {
    provider: {
      nextroute: {
  ...
  const nextroute = (result.provider as Record<string, unknown>).nextroute as { name: string };
  assert.equal(nextroute.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "nextroute/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "nextroute/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_nextroute" }
  );
  assert.ok(!("model" in result));
  assert.ok(!("small_model" in result));
});

test("NEXTROUTE_MCP_DEFAULT_SCOPES contains 7 read-only scopes", () => {
  assert.equal(NEXTROUTE_MCP_DEFAULT_SCOPES.length, 7);
  assert.ok(NEXTROUTE_MCP_DEFAULT_SCOPES.every((s) => s.startsWith("read:")));
});

test("createNextRouteMCPEntry defaults to tsx runtime", () => {
  const entry = createNextRouteMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_nextroute",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.NEXTROUTE_API_KEY, "sk_nextroute");
  assert.ok(!("NEXTROUTE_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("NEXTROUTE_MANAGEMENT_API_KEY" in entry.env));
});

test("createNextRouteMCPEntry uses node runtime when specified", () => {
  const entry = createNextRouteMCPEntry({
...
test("createNextRouteMCPEntry sets management key and scopes when supplied", () => {
  const entry = createNextRouteMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_nextroute",
  ...
  assert.equal(entry.env.NEXTROUTE_MANAGEMENT_API_KEY, "sk_manage");
  assert.equal(entry.env.NEXTROUTE_MCP_ENFORCE_SCOPES, "true");
  assert.equal(entry.env.NEXTROUTE_MCP_SCOPES, "read:health,read:combos,execute:completions");
});

test("createNextRouteMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createNextRouteMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(
    () => createNextRouteMCPEntry({ serverPath: "/p", apiKey: "" }),
    /apiKey is required/
  );
});
...
test("createNextRouteComboConfig builds minimal payload", () => {
  const payload = createNextRouteComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createNextRouteComboConfig includes optional fields when supplied", () => {
...
test("NEXTROUTE_DEFAULT_OPENCODE_MODELS includes cc/ prefixed models", () => {
  const defaults = [...NEXTROUTE_DEFAULT_OPENCODE_MODELS];
...
test("NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS covers every default model id", () => {
  for (const id of NEXTROUTE_DEFAULT_OPENCODE_MODELS) {
    const ctx = NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.ok(
...
test("createNextRouteProvider emits limit.context on default model entries", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });
...
test("createNextRouteProvider omits limit.context for unknown model ids", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
...
test("createNextRouteProvider serialises limit.context to JSON", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });
  ...
  for (const id of NEXTROUTE_DEFAULT_OPENCODE_MODELS) {
    const expectedContext = NEXTROUTE_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.equal(
...
test("NEXTROUTE_DEFAULT_MODEL_CAPABILITIES covers every default model id", () => {
  for (const id of NEXTROUTE_DEFAULT_OPENCODE_MODELS) {
    const caps = NEXTROUTE_DEFAULT_MODEL_CAPABILITIES[id];
...
test("createNextRouteProvider emits default capability flags inline with the model entry", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
  });
...
test("createNextRouteProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
    modelCapabilities: {
...
test("createNextRouteProvider applies capability overrides to non-default model ids", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
    models: ["custom-model"],
...
test("createNextRouteProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createNextRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_nextroute",
...
test("createNextRouteAgentBlock builds provider-prefixed entries per role", () => {
  const block = createNextRouteAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "nextroute/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "nextroute/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "nextroute/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createNextRouteAgentBlock omits optional fields when not supplied", () => {
  const block = createNextRouteAgentBlock({
...
test("createNextRouteAgentBlock skips roles with empty modelId", () => {
  const block = createNextRouteAgentBlock({
    roles: {
...
test("createNextRouteAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createNextRouteAgentBlock({
    roles: {
      build: {
...
test("createNextRouteAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createNextRouteAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createNextRouteModesBlock builds provider-prefixed mode entries", () => {
  const block = createNextRouteModesBlock({
    modes: {
...
test("createNextRouteModesBlock skips modes with empty modelId", () => {
  const block = createNextRouteModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createNextRouteModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createNextRouteModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",

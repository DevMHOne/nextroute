...
import {
  ...
  diskSnapshotPath,
  type NextRouteFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<NextRouteFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-disk-perms-"));

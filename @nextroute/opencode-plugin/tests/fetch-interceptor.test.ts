 ...
 *
 * Covers `createNextRouteFetchInterceptor` (URL-prefix gating, header merge,
...
import { createNextRouteAuthHook, createNextRouteFetchInterceptor } from "../src/index.js";
...
test("createNextRouteFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createNextRouteFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createNextRouteFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createNextRouteFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({
      apiKey: KEY,
      baseURL: `${BASE}////`,
    });
    await f(`${BASE}/models`, {});
...
test("createNextRouteFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createNextRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createNextRouteAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createNextRouteAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createNextRouteAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);

 ...
 *
 * Covers the `createNextRouteAuthHook(opts)` factory and its loader behaviour
...
import { createNextRouteAuthHook } from "../src/index.js";

test("createNextRouteAuthHook: default providerId is 'nextroute'", () => {
  const hook = createNextRouteAuthHook();
  assert.equal(hook.provider, "nextroute");
});

test("createNextRouteAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createNextRouteAuthHook({ providerId: "nextroute-preprod" });
  assert.equal(hook.provider, "nextroute-preprod");
});

test("createNextRouteAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createNextRouteAuthHook();
  ...
  assert.equal(m.label, "NextRoute API Key");

  const custom = createNextRouteAuthHook({ providerId: "nextroute-preprod" });
  assert.equal(custom.methods[0].label, "NextRoute (nextroute-preprod) API Key");
});

test("createNextRouteAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("nextroute"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createNextRouteAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(

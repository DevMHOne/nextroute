 ...
 *
 * Covers `createNextRouteProviderHook(opts, deps)`:
...
const FIXTURE: NextRouteRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: NextRouteRawModelEntry[]): NextRouteModelsFetcher & {
  callCount: () => number;
...
test("createNextRouteProviderHook: default providerId is 'nextroute'", () => {
  const hook = createNextRouteProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "nextroute");
});

test("createNextRouteProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createNextRouteProviderHook(
    { providerId: "nextroute-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createNextRouteProviderHook(
    { providerId: "nextroute-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "nextroute-preprod");
  assert.equal(b.id, "nextroute-local");
});
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createNextRouteProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  assert.equal(claude.providerID, "nextroute");
  ...
  // cost is zeroed (NextRoute /v1/models has no pricing)

 ...
 *
 * Validates that two `NextRoutePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@nextroute/opencode-plugin", {"providerId": "nextroute-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@nextroute/opencode-plugin", {"providerId": "nextroute-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { NextRoutePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof NextRoutePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await NextRoutePlugin(fakeInput, {
    providerId: "nextroute-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await NextRoutePlugin(fakeInput, {
    providerId: "nextroute-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "nextroute-prod");
  assert.equal(b.auth?.provider, "nextroute-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await NextRoutePlugin(fakeInput, { ...opts });
  const second = await NextRoutePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => NextRoutePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await NextRoutePlugin(fakeInput, {

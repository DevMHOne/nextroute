 ...
 *
 * Surfaces tested:
 *   - `parseNextRoutePluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createNextRouteProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createNextRouteConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultNextRouteEnrichmentFetcher,
  defaultNextRouteCompressionMetaFetcher,
  ...
  type NextRouteCompressionCombo,
...
test("parseNextRoutePluginOptions: empty features object → preserved", () => {
  const r = parseNextRoutePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseNextRoutePluginOptions: all boolean features set → preserved", () => {
  const r = parseNextRoutePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseNextRoutePluginOptions: mcpToken string → preserved", () => {
  const r = parseNextRoutePluginOptions({
...
test("parseNextRoutePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseNextRoutePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @nextroute\/opencode-plugin options/
  );
});

test("parseNextRoutePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseNextRoutePluginOptions({
...
test("parseNextRoutePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseNextRoutePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @nextroute\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: NextRouteRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  let called = 0;
  const enrichment: NextRouteEnrichmentMap = new Map([
    ["claude-sonnet-4-6", { name: "Claude Sonnet 4.6", pricing: { input: 3, output: 15 } }],
  ]);
  const hook = createNextRouteProviderHook(
    { providerId: "nextroute", baseURL: "https://or.example.com/v1" },
    {
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: NextRouteCompressionCombo[] = [
    {
      id: "default-caveman",
...
const stubAuthJson = (apiKey: string) => async () => ({
  nextroute: { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.nextroute, "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.nextroute as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.nextroute as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { nextroute: { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.nextroute,
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createNextRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "nextroute-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["nextroute-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultNextRouteEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultNextRouteEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultNextRouteEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultNextRouteEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultNextRouteCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultNextRouteCompressionMetaFetcher("", "sk", 100);
...
test("defaultNextRouteEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultNextRouteEnrichmentFetcher(
...
test("defaultNextRouteEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultNextRouteEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultNextRouteEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): NextRouteEnrichmentMap {
  const map: NextRouteEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: NextRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as NextRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as NextRouteRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: NextRouteRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as NextRouteRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: NextRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as NextRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as NextRouteRawModelEntry,
    { id: "cx/gpt-5.5" } as NextRouteRawModelEntry,
    { id: "codex/gpt-5.5" } as NextRouteRawModelEntry,
    { id: "pol/openai-large" } as NextRouteRawModelEntry,
    { id: "pollinations/openai-large" } as NextRouteRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);

 ...
 *
 * Covers:
 *   - `defaultNextRouteCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createNextRouteProviderHook(opts, deps)` extension
...
const MODEL_PRIMARY: NextRouteRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: NextRouteRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: NextRouteRawModelEntry = {
  id: "gemini-3-flash",
...
): NextRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): NextRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultNextRouteCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultNextRouteCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultNextRouteCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultNextRouteCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultNextRouteCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultNextRouteCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultNextRouteCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultNextRouteCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultNextRouteCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultNextRouteCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultNextRouteCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultNextRouteCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(
    async () => defaultNextRouteCombosFetcher("", "sk-test"),
    /baseURL required/
  );
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "nextroute-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: NextRouteRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createNextRouteProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.equal(combo.providerID, "nextroute");
...
test("models(): combo name exactly matches raw model id → raw deleted, combo lives at combo/ key, no warn", async () => {
  ...
  // the real NextRoute payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: NextRouteRawCombo = {
    id: "uuid-collider",
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  const combos: NextRouteRawCombo[] = [
    {
      id: "uuid-a",
      name: "Claude",

 ...
 *
 * Covers `createNextRouteConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@nextroute/opencode-provider`).
...
import {
  ...
  createNextRouteConfigHook,
  createNextRouteProviderHook,
  NextRoutePlugin,
  resolveNextRoutePluginOptions,
  type NextRouteCombosFetcher,
  type NextRouteEnrichmentEntry,
  type NextRouteEnrichmentFetcher,
  type NextRouteEnrichmentMap,
  type NextRouteFetchCache,
  type NextRouteModelsFetcher,
  type NextRouteProviderConnection,
  type NextRouteProvidersFetcher,
  type NextRouteRawCombo,
  type NextRouteRawModelEntry,
  type NextRouteReadAuthJson,
  type NextRouteStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: NextRouteRawModelEntry = {
...
const MODEL_GEMINI: NextRouteRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: NextRouteRawCombo = {
...
): NextRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: NextRouteReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): NextRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: NextRouteReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: NextRouteRawModelEntry[]
): NextRouteModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NextRouteModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: NextRouteRawCombo[]
): NextRouteCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NextRouteCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): NextRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: NextRouteModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): NextRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: NextRouteCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: NextRouteEnrichmentMap): NextRouteEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: NextRouteEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): NextRouteEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: NextRouteEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createNextRouteConfigHook(
  ...
  const provider = (input as { provider: Record<string, NextRouteStaticProviderEntry> }).provider;
  const entry = provider.nextroute;
  assert.ok(entry, "input.provider.nextroute set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "NextRoute");
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  ...
  const input = makeInput({ nextroute: manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider.nextroute, manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);

  const entry = (input as { provider: Record<string, NextRouteStaticProviderEntry> }).provider
    .nextroute;
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  ...
  const hookA = createNextRouteConfigHook(
    { providerId: "nextroute-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createNextRouteConfigHook(
    { providerId: "nextroute-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, NextRouteStaticProviderEntry> }).provider;
  assert.ok(provider["nextroute-prod"], "prod block present");
  assert.ok(provider["nextroute-preprod"], "preprod block present");
  assert.equal(provider["nextroute-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["nextroute-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["nextroute-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(provider["nextroute-preprod"].options.baseURL, "https://preprod.example/v1");
  assert.notEqual(
    provider["nextroute-prod"],
    provider["nextroute-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createNextRouteConfigHook(
    { providerId: "nextroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createNextRouteProviderHook(
    { providerId: "nextroute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: NextRouteFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @nextroute/opencode-provider", () => {
  const resolved = resolveNextRoutePluginOptions({
    providerId: "nextroute",
    displayName: "NextRoute",
  });
  ...
  assert.equal(block.name, "NextRoute");
...
test("buildStaticProviderEntry: empty fetch results → stub block with models: {}", () => {
  const resolved = resolveNextRoutePluginOptions({ providerId: "nextroute" });
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveNextRoutePluginOptions({ providerId: "nextroute" });
  const withDate: NextRouteRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
...
test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveNextRoutePluginOptions({ providerId: "nextroute" });
  const TEXT_ONLY: NextRouteRawModelEntry = {
    id: "text-only",
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: NextRoutePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("NextRoutePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await NextRoutePlugin({} as never, { providerId: "nextroute" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    nextroute: { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, NextRouteEnrichmentEntry>([
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
function stubProvidersFetcher(payload: NextRouteProviderConnection[]): NextRouteProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: NextRouteProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): NextRouteProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: NextRouteProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: NextRouteRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: NextRouteRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot NextRoute hasn't curated a human label for yet. We still
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createNextRouteConfigHook(
    { providerId: "nextroute", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, NextRouteStaticProviderEntry> }).provider
    .nextroute;
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, NextRouteStaticProviderEntry> }).provider
    .nextroute;

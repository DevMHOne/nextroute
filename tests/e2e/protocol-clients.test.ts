...
const BASE_URL = process.env.NEXTROUTE_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.NEXTROUTE_API_KEY || "";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          NEXTROUTE_BASE_URL: BASE_URL,
          NEXTROUTE_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("nextroute_get_health");
        expect(toolNames).toContain("nextroute_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=nextroute_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "nextroute_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );

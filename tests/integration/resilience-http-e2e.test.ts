...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-resilience-http-e2e-"));
...
function createServerProcess(dataDir: string, port: number) {
  ...
  const child = spawn(process.execPath, ["scripts/dev/run-next-playwright.mjs", "dev"], {
    ...
    env: {
      ...
      NEXTROUTE_DISABLE_TOKEN_HEALTHCHECK: "true",
      NEXTROUTE_DISABLE_LOCAL_HEALTHCHECK: "true",
      NEXTROUTE_HIDE_HEALTHCHECK_LOGS: "true",
      NEXTROUTE_E2E_BOOTSTRAP_MODE: "open",
    },
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 120_000) {
    if (logs.exitInfo) {
      throw new Error(
        [
          `NextRoute exited before it became ready (code=${logs.exitInfo.code}, signal=${logs.exitInfo.signal})`,
          "--- stdout ---",
  ...
  throw new Error(
    [
      `Timed out waiting for NextRoute to start: ${lastError}`,
      "--- stdout ---",
      ...logs.stdoutLines.slice(-40),
      "--- stderr ---",
      ...logs.stderrLines.slice(-40),

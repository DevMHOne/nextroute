...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-smelt-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "nextroute");
...
// ── Test 5: DELETE → removes NextRoute fields ────────────────────────────────

test("smelt-settings DELETE: removes NextRoute fields from existing config", async () => {

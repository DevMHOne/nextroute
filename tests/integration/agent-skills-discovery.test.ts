 ...
 *  3. MCP tool nextroute_agent_skills_list handler returns 42 entries.
...
// ── §3: MCP tool nextroute_agent_skills_list ─────────────────────────────────

test("MCP nextroute_agent_skills_list handler returns count 42", async () => {
...
test("MCP nextroute_agent_skills_list result has all 42 IDs", async () => {
  const result = await agentSkillTools.nextroute_agent_skills_list.handler({});

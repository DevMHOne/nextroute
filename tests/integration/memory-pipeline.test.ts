...
test("successful responses extract facts and persist them as memories", async () => {
  ...
  const response = await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-nextroute-session-id": "session-extract" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
...
test("later requests inject retrieved memories into upstream messages", async () => {
  ...
  const response = await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-nextroute-session-id": "session-inject" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
...
test("memory search ranks query-relevant memories first", async () => {
  ...
  await memoryTools.nextroute_memory_add.handler({
    apiKeyId: apiKey.id,
    sessionId: "search",
    type: "factual",
    key: "pref:language",
  ...
  const result = await memoryTools.nextroute_memory_search.handler({
    apiKeyId: apiKey.id,
...
test("memory clear removes all stored memories for an API key", async () => {
  ...
  const cleared = await memoryTools.nextroute_memory_clear.handler({
    apiKeyId: apiKey.id,
  });
...
test("extracted memories remain isolated by session id", async () => {
  ...
  await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-nextroute-session-id": "session-a" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
  ...
  await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-nextroute-session-id": "session-b" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,

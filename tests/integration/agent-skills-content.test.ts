 ...
 *  2. Zero nextroute-* folders remain (post-prune: old nextroute-* skill dirs were removed).
...
// ── §2: No nextroute-* directories remain ────────────────────────────────────

test("skills/ has zero nextroute-* directories (all pruned)", () => {
  ...
  assert.deepEqual(
    omniRouteDirs,
    [],
    `Found nextroute-* directories that should have been pruned: ${omniRouteDirs.join(", ")}`,
  );
});

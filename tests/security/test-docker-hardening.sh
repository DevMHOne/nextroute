...
IMAGE_NAME="${1:-nextroute-local-hardened}"
CONTAINER_NAME="${2:-nextroute-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/nextroute_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/nextroute_hardening_container_id.txt

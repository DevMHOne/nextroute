...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-chat-helpers-"));
...
test("checkPipelineGates blocks providers with an open circuit breaker", async () => {
  ...
  assert.equal(response.headers.get("X-NextRoute-Provider-Breaker"), "open");
});
...
test("withSessionHeader adds headers to mutable and immutable responses", async () => {
  ...
  assert.equal(mutable.headers.get("X-NextRoute-Session-Id"), "sess_mutable");
  assert.equal(immutable.headers.get("X-NextRoute-Session-Id"), "sess_redirect");

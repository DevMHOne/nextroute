...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "nextroute-autostart-linux-"));
...
test("resolveCliPath finds nextroute.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "nextroute.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "nextroute.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*nextroute\.mjs.*serve --no-open/m);

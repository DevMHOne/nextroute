...
function cleanupGlobalDb() {
  try {
    if (globalThis.__nextrouteDb?.open) {
      globalThis.__nextrouteDb.close();
    }
  } catch {}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("nextroute-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _nextroute_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("nextroute-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("nextroute-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "nextroute")
              : path.join(fakeHome, ".nextroute");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("nextroute-db-build-");
...
test("getDbInstance surfaces invalid DATA_DIR paths as sqlite open failures", serial, async () => {
  const sandboxDir = makeTempDir("nextroute-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-legacy-empty-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      INSERT INTO _nextroute_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _nextroute_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("nextroute-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");

...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__nextrouteDb?.open) {
      (globalThis as any).__nextrouteDb.close();
    }
  } catch {}
  delete (globalThis as any).__nextrouteDb;
}

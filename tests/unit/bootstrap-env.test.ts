...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".nextroute") });
  } finally {
...
test("bootstrapEnv prefers ~/.nextroute/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {

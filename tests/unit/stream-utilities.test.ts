...
test("wantsProgress detects X-NextRoute-Progress header correctly", () => {
  const headersObj = { "x-nextroute-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-nextroute-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-nextroute-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-nextroute-progress": "false" }), false);

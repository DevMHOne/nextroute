...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-command-code-auth-"));
...
test.beforeEach(() => {
  delete process.env.NEXTROUTE_PUBLIC_BASE_URL;
  delete process.env.NEXTROUTE_BASE_URL;
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.NEXTROUTE_PUBLIC_BASE_URL = "https://nextroute.example.com/base-path";

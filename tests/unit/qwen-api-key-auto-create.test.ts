...
const DUMMY_HOME = path.join(os.tmpdir(), "nextroute-qwen-key-test-" + Date.now());
...
test.afterEach(async () => {
  ...
  if (process.env.DATA_DIR?.includes("nextroute-qwen-key-test")) {
...
test("getOrCreateApiKey() creates DB-backed key when no keyId provided", async () => {
  ...
  // Key should NOT be the placeholder "sk_nextroute"
  assert.notEqual(apiKey, "sk_nextroute", "Should not return placeholder");
...
test("Qwen guide-settings POST creates valid DB-backed key (no keyId)", async () => {
  ...
  assert.notEqual(apiKey, "sk_nextroute", "Should not use placeholder");
  assert.ok(apiKey.startsWith("sk-"), "Key should start with sk- prefix");

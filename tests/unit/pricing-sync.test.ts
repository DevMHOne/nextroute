...
import { transformToNextRoute } from "../../src/lib/pricingSync.ts";

// ─── transformToNextRoute ────────────────────────────────

describe("transformToNextRoute", () => {
  test("converts LiteLLM per-token pricing to NextRoute per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToNextRoute(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToNextRoute(raw);

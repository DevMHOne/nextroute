...
test("electron smoke discovers the default Linux executable name", () => {
  assert.ok(LINUX_EXECUTABLE_NAMES.includes("nextroute-desktop"));
});

test("electron smoke env allowlists runtime variables and drops secrets", () => {
  const env = buildSmokeEnv({
    currentPlatform: "linux",
    dataDir: "/tmp/nextroute-electron-smoke-test",
    parentEnv: {
      DISPLAY: ":99",
  ...
  assert.equal(env.DATA_DIR, "/tmp/nextroute-electron-smoke-test");
  assert.equal(env.DISPLAY, ":99");
  assert.equal(env.PATH, "/usr/bin");
  assert.equal(env.HOME, "/tmp/nextroute-electron-smoke-test/home");
  assert.equal(env.XDG_CONFIG_HOME, "/tmp/nextroute-electron-smoke-test/config");

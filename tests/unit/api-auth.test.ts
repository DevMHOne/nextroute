...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-api-auth-"));
...
test("getApiKeyMetadata recognizes NEXTROUTE_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.NEXTROUTE_API_KEY = envKey;
...
test("requireManagementAuth returns null for NEXTROUTE_API_KEY env passthrough", async () => {
  await setupAuth();

...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-fake-bin-"));
...
test("install creates package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "nextroute-9router-host");

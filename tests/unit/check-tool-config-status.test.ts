...
process.env.DATA_DIR = path.join(os.tmpdir(), "nextroute-check-tool-test");
...
async function writeTempFile(filename: string, content: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nextroute-clicheck-"));
...
async function writeCodexConfig(opts: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nextroute-codex-"));
  const configPath = path.join(tmpDir, "config.toml");

  const tomlContent = opts.hasNextRoute
...
test("codex: returns 'configured' when TOML has NextRoute URL + valid auth key", async () => {
  const configPath = await writeCodexConfig({
    hasNextRoute: true,
    authApiKey: "sk_nextroute_testkey_1234567890abcdef",
  });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "configured");
});

test("codex: returns 'not_configured' when TOML has NextRoute URL but auth key is masked", async () => {
  const configPath = await writeCodexConfig({
    hasNextRoute: true,
    authApiKey: "sk_****",
  });
...
test("codex: returns 'not_configured' when TOML does not mention NextRoute", async () => {
  const configPath = await writeCodexConfig({ hasNextRoute: false });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "not_configured");
});
...
test("qwen: returns 'configured' when modelProviders has NextRoute URL", async () => {
  const configPath = await writeTempFile(
    "qwen.json",
    JSON.stringify({
      modelProviders: [{ apiBase: "http://localhost:20128/v1", name: "nextroute" }],
    })
  );
...
test("hermes: returns 'configured' when config contains NextRoute", async () => {
  const configPath = await writeTempFile(
    "hermes.toml",
    `[openai]\nbase_url = "http://localhost:20128/v1"\n`
  );
...
test("droid: returns 'configured' when JSON config contains sk_nextroute marker", async () => {
  const configPath = await writeTempFile(
    "droid.json",
    JSON.stringify({ apiKey: "sk_nextroute_somekey", baseUrl: "http://localhost:20128/v1" })
  );
  const result = await checkToolConfigStatus("droid", configPath);
  assert.equal(result, "configured");
});

test("openclaw: returns 'configured' when JSON config contains nextroute text", async () => {
  const configPath = await writeTempFile(
    "openclaw.json",
    JSON.stringify({ openAiBaseUrl: "http://nextroute.local/v1", openAiApiKey: "sk-test" })
  );
...
test("kilo: returns 'not_configured' when no NextRoute markers present", async () => {
  const configPath = await writeTempFile(
    "kilo.json",

...
test("C2 header contract — server.cjs intercept must inject x-nextroute-source and x-nextroute-agent", async () => {
  ...
  assert.match(
    src,
    /"x-nextroute-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-nextroute-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-nextroute-agent":\s*agentId/,
    'server.cjs must inject "x-nextroute-agent: <id>" from the host→agent map'
  );

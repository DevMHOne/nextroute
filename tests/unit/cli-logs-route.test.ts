...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-cli-logs-route-"));
...
test("GET /api/cli-tools/logs returns empty array when log file does not exist", async () => {
  ...
  process.env.APP_LOG_FILE_PATH = "/tmp/nextroute-nonexistent-cli-logs-test.log";

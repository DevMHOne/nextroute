...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/nextroute" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/nextroute");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/nextroute",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/nextroute/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "nextroute-base",
    ...
    assert.equal(config.repoDir, "/srv/nextroute");
    assert.equal(config.composeFile, "/srv/nextroute/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "nextroute-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart nextroute --update-env \|\| true/);
  });
});

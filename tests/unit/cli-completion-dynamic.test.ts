...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("nextroute"), "bash script should mention nextroute");
  assert.ok(out.includes("_nextroute"), "bash script should define _nextroute function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("nextroute"), "fish script should mention nextroute");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("nextroute_get_cache"),

...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_nextroute when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_nextroute");
  });

  it("falls back to sk_nextroute when both are undefined", async () => {
    mockKeyStore.clear();

...
const {
  ...
  NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME,
...
// nextroute_web_search tool must use the FLAT function shape ({ type, name }), not the
...
test("#2390 web_search fallback is FLAT for Responses API target", () => {
  ...
  assert.equal(injected.name, NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.function,
    undefined,
...
test("#2390 web_search fallback stays NESTED for Chat Completions target", () => {
  ...
  assert.equal(fn?.name, NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.name,
    undefined,
...
test("#2390 tool_choice matches the injected tool shape per target format", () => {
  ...
  assert.equal(rChoice.name, NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.equal(cFn?.name, NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
});
...
test("OpenAI -> Claude (non-passthrough): built-in web_search IS still converted", () => {
  ...
  assert.equal(fallback.enabled, true);
  assert.equal(fallback.toolName, NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.ok(toolNames.includes(NEXTROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME));
});

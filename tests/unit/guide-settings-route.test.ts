...
const DUMMY_HOME = path.join(os.tmpdir(), "nextroute-qwen-test-" + Date.now());
...
test("guide-settings POST creates new hermes config.yaml if it doesn't exist", async () => {
  ...
  assert.equal(content.model?.provider, "nextroute");
  assert.equal(content.model?.base_url, "http://my-omni/v1");
  assert.equal(content.providers?.nextroute?.base_url, "http://my-omni/v1");
  assert.ok(String(content.providers?.nextroute?.api_key || "").startsWith("sk-"));
});
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider.nextroute.npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider.nextroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.nextroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider.nextroute.models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.nextroute", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider.nextroute.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.nextroute.options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider.nextroute.models, {

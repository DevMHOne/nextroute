...
test("AntigravityExecutor.buildHeaders includes native headers without NextRoute internals", () => {
  const executor = new AntigravityExecutor();
  ...
  assert.equal(headers["X-NextRoute-Source"], undefined);
});

test("Antigravity header scrub removes NextRoute internal headers", () => {
  const headers = scrubProxyAndFingerprintHeaders({
    Authorization: "Bearer ag-token",
    "X-NextRoute-Source": "nextroute",
  ...
  assert.equal(headers.Authorization, "Bearer ag-token");
  assert.equal(headers["X-NextRoute-Source"], undefined);
  assert.equal(headers["X-NextRoute-No-Cache"], undefined);
...
test("AntigravityExecutor.transformRequest allows body project overrides when the env flag is enabled", async () => {
  ...
  await withEnv("NEXTROUTE_ALLOW_BODY_PROJECT_OVERRIDE", "1", async () => {
    const result = await executor.transformRequest(
      "antigravity/gemini-2.5-pro",
      {
        project: "body-project",

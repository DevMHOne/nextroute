...
test("isCodexResponsesWebSocketRequired: NEXTROUTE_CODEX_WS_ENABLED=false forces HTTP even with codexTransport=websocket", () => {
  ...
  const prev = process.env.NEXTROUTE_CODEX_WS_ENABLED;
  process.env.NEXTROUTE_CODEX_WS_ENABLED = "false";
  try {
    ...
    if (prev === undefined) delete process.env.NEXTROUTE_CODEX_WS_ENABLED;
    else process.env.NEXTROUTE_CODEX_WS_ENABLED = prev;
    __setCodexWebSocketTransportForTesting(undefined);
  }
});
...
test("CodexExecutor.transformRequest preserves store-enabled responses state when explicitly enabled", () => {
  ...
  assert.equal(result._nextrouteResponsesStore, undefined);
...
test("Codex internal websocket bridge rejects non-object JSON payloads", async () => {
  await withEnv({ NEXTROUTE_WS_BRIDGE_SECRET: "bridge-secret" }, async () => {
    const { POST } = await import("../../src/app/api/internal/codex-responses-ws/route.ts");

    const response = await POST(
      new Request("http://nextroute.local/api/internal/codex-responses-ws", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-nextroute-ws-bridge-secret": "bridge-secret",
        },

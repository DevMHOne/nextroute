...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "nextroute-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@nextroute\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@nextroute\/open-sse\/services\/model\.ts/);

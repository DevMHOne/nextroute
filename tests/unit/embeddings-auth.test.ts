...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.NEXTROUTE_API_KEY;

  await t.test("should return true if key matches NEXTROUTE_API_KEY", async () => {
    process.env.NEXTROUTE_API_KEY = "test-key";
    assert.strictEqual(await isValidApiKey("test-key"), true);
    delete process.env.NEXTROUTE_API_KEY;
  });
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.NEXTROUTE_API_KEY = originalOmniKey;
});

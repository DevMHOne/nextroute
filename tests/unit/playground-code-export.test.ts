...
function assertSecurityInvariants(generated: string, label: string) {
  assert.ok(generated.includes(API_KEY_PLACEHOLDER), `${label}: must include $NEXTROUTE_API_KEY`);
...
test("API_KEY_PLACEHOLDER is $NEXTROUTE_API_KEY", () => {
  assert.equal(API_KEY_PLACEHOLDER, "$NEXTROUTE_API_KEY");
});
...
test("chat.completions × python: contains required elements", () => {
  ...
  assert.ok(generated.includes('os.environ["NEXTROUTE_API_KEY"]'), "uses os.environ");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});
...
test("chat.completions × typescript: contains required elements", () => {
  ...
  assert.ok(generated.includes("process.env.NEXTROUTE_API_KEY"), "uses process.env");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});

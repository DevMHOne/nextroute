...
function createTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "nextroute-sync-env-"));
}
...
test("syncEnv creates .env from .env.example and generates install-time secrets", () => {
  ...
  // find the user's real DB at ~/.nextroute/ during tests
  ...
  try {
    ...
    assert.match(envContent, /^MACHINE_ID_SALT=nextroute-/m);

...
test("builds compact OpenAI stream summary for detailed logs", () => {
  ...
  assert.equal(compact._nextroute_stream.stage, "provider_response");
  assert.equal(compact._nextroute_stream.eventCount, 3);
...
test("builds compact Claude stream summary for detailed logs", () => {
  ...
  assert.equal(compact._nextroute_stream.eventCount, 4);
});

...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_nextroute',
...
test("sentinel guard condition catches sk_nextroute and null", () => {
  const SENTINEL = "sk_nextroute";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_nextroute')
  ...
  assert.equal(shouldReject("sk_nextroute"), true, "sentinel must be rejected");

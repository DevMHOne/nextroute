...
Run NextRoute with Podman via **Quadlet** (systemd integration) or **podman compose**.
...
podman build --target runner-web -t nextroute:web .
# For CLI tool support:
podman build --target runner-cli -t nextroute:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/nextroute
cp contrib/podman/*.container ~/.config/containers/systemd/nextroute/
cp contrib/podman/*.network ~/.config/containers/systemd/nextroute/
cp contrib/podman/*.volume ~/.config/containers/systemd/nextroute/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/nextroute/nextroute.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/NextRoute/.env
```
...
systemctl --user start nextroute-redis
systemctl --user start nextroute
```

### 5. Verify

```bash
systemctl --user status nextroute
...
```bash
journalctl --user -u nextroute -f
```

### 6. Enable on boot

```bash
systemctl --user enable nextroute-redis
systemctl --user enable nextroute
```
